...
$(document).ready(function() {
  ...
  function setupMobileMenu() {
    ...
    require(['pulling', 'storage'], function(Pulling, Storage) {
      ...
      var chatMenuVisible =
        !config.disableChat && app.user && parseInt(app.user.uid, 10);
      ...
      function loadNotifications() {
        require(['notifications'], function(notifications) {
          notifications.loadNotifications(
            $('#menu [data-section="notifications"] ul'),
          );
        });
      }
      ...
      function setupSetting() {
        if (
          ajaxify.data.template['account/settings'] &&
          !document.getElementById('persona:menus:legacy-layout')
        ) {
          require(['translator'], function(translator) {
            translator.translate('[[persona:mobile-menu-side]]', function(
              translated,
            ) {
              $(
                '<div class="well checkbox"><label><input type="checkbox" id="persona:menus:legacy-layout"/><strong>' +
                  translated +
                  '</strong></label></div>',
              )
                ...
                .prop(
                  'checked',
                  Storage.getItem('persona:menus:legacy-layout', 'true'),
                )
                .change(function(e) {
                  if (e.target.checked) {
  ...
  function generateUserCard(ev) {
    ...
    socket.emit('user.isFollowing', { uid: data.uid }, function(
      err,
      isFollowing,
    ) {
  ...
  function customization() {
      ...
      fetch(`${location.origin}/api`)
        ...
        .then(data => {
          categories = (data.categories || []).map(({ name, icon, slug }) => {
            const html = `<li><a class="navigation-link" href="/category/${slug}" title="" data-original-title="${name}"><i class="fa fa-fw ${icon}" data-content=""></i><span class="visible-xs-inline"> ${name}</span></a></li>`;
    ...
    setTimeout(() => {
      // B3. reply features
      // expand reply if existed
      var buttonName = $('[component="post/reply-count"] i').not('.hidden').attr('component')
      if (buttonName === 'post/replies/open') {
        $($('[component="post/reply-count"]').get(0)).trigger('click');
      }
    }, 1000)

    setInterval(() => {
      // B3. reply features
      // hide redundant reply buttons
      $('.post-footer > small.pull-right span.post-tools').slice(1)
        .each(function() { $(this).hide() });
      // hide redundant post bar
      $('.post-bar').slice(1).each(function() { $(this).hide() });
      // hide redundant rectangle reply button
      $('.topic-main-buttons > div[component="topic/reply/container"]').slice(1)
        .each(function() {  $(this).hide() });
      // hide redundant post
      const inThreadReplyPostIDs = []
      $('ul.posts div[component="post/replies"] > li').each(function() { inThreadReplyPostIDs.push($(this).attr('data-pid')) });
      $('ul.posts > li[component="post"]').slice(1).each(function() {
        const pid = $(this).attr('data-pid')
        if (inThreadReplyPostIDs.includes(pid)) $(this).hide()
      });

      // B4. private reply features
      // hide all private replies
      var username = app.user.username
      var firstEditorUsername = $('ul[component="topic"]:first > li[component="post"]:first').attr('data-username')
      if (username !== firstEditorUsername && username !== ADMIN_USERNAME) {
        var hidePost = function() {
          if ((this.textContent || '').replace(/^\s+/gm,'').slice(0,3) === '＊＊＊') {
            $(this).parent().hide();  // hide parent li element
          }
        }
        var inThreadPostQueries = $('div[component="post/replies"] > li[component="post"]')
        if (username) {
          inThreadPostQueries = inThreadPostQueries.not(`[data-username="${username}"]`)
        }
        inThreadPostQueries.find('div[component="post/content"]').each(hidePost);

        // other posts
        var notInThreadPostQueries = $('ul.posts > li[component="post"]').slice(1)
        if (username) {
          notInThreadPostQueries = notInThreadPostQueries.not(`[data-username="${username}"]`)
        }
        notInThreadPostQueries.find('div[component="post/content"]').each(hidePost);
      }
      ...
      if ((location.pathname || '').match(/^\/user/g)) {
        // hide custom sidebar
        $('ul._sidebar').hide()
        // revert content class
        $('#content').addClass('col-lg-12').removeClass('col-lg-9')
      } else {
        $('ul._sidebar').show()
        $('#content').addClass('col-lg-9').removeClass('col-lg-12')
      }
